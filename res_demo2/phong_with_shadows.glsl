...
float directional_shadows(vec4 frag_pos_in_light_space)
{
    ...
    //float bias = 0.002f;
    float bias = max(0.001f * (1.0 - dot(normalize(NORMAL), normalize(directional_lights[0].direction))), 0.0001f);
...
void main() {
    ...
    result.xyz = (BASE_AMBINET_LIGHT * material.ambient) + (1.0 - directional_shadows(FRAG_LIGHT_SPACE_POS)) * ((lighting[1] * material.diffuse) + (lighting[2] * material.specular));
